...
from models.att.GLSA import GLSA
#-------------------------------------------------------------------------------
import pywt
import pywt.data
import torch
...
from models.conv1.AKconv import AKConv
__all__ = ['RFM']

from models.conv1.C2f_Dual import C2f_Dual
...
from models.att.CBAM import CBAM
...
class MSHV_Net(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list=[8, 16, 24, 32, 48, 64],
        ...
        if bridge:
            self.ACF1 = GLSA(c_list[0],c_list[0])
            self.ACF2 = GLSA(c_list[1],c_list[1])
            self.ACF3 = GLSA(c_list[2],c_list[2])
            self.ACF4 = GLSA(c_list[3],c_list[3])
            self.ACF5 = GLSA(c_list[4],c_list[4])
