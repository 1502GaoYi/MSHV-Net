...
from models.att.GAM_Attention import GAM_Attention
...
class MSHV_Net(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list=[8, 16, 24, 32, 48, 64],
        ...
        if bridge:
            self.ACF1 = GAM_Attention(c_list[0])
            self.ACF2 = GAM_Attention(c_list[1])
            self.ACF3 = GAM_Attention(c_list[2])
            self.ACF4 = GAM_Attention(c_list[3])
            self.ACF5 = GAM_Attention(c_list[4])
