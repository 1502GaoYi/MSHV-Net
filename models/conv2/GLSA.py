...
"""
BasicConv2d：一个简单的2D卷积层，后接批量归一化和ReLU激活函数。
ContextBlock：一个执行上下文聚合的模块，使用平均池化或注意力池化，然后进行通道加法或乘法。
ConvBranch：一个卷积分支，通过多个卷积层处理输入特征并应用空间注意力机制。
GLSA（Global-to-Local Spatial Aggregation）：这个模块将输入分为两部分，分别使用局部分支和全局上下文模块进行处理，然后将结果结合。
"""
...
if __name__ == '__main__':
    # 假设输入的尺寸为 (batch_size, channels, height, width)
    input_tensor = torch.randn(1, 8, 64, 64)

    # 实例化GLSA模块
    glsa = GLSA(input_dim=8, embed_dim=8)

    # 前向传播并打印输入和输出的形状
    output_tensor = glsa(input_tensor)
